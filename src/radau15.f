...
C  Integration is in double precision. A 64-bit double-word is assumed.
      ...
      REAL *4 TVAL,PW
      DIMENSION X(3),V(3),F1(3),FJ(18),C(21),D(21),R(21),Y(18),Z(18),
...
C  values from the predicted B-values, following Eq. (2.7) in text.
 722  DO 58 K=1,NV
  ...
  58  G(7,K)=                                                   B(7,K)
      ...
      IF(NCL) T2=T
      TVAL=DABS(T)
c      IF(NS/10*10.EQ.NS) WRITE(*,7) NF,NS,X(1),X(2),T,TM,TF
...
c      IF(NCOUNT.GT.1) WRITE (4,8) NOR,NCOUNT,T,TP
...
c      WRITE(*,7) NF,NS,X(1),X(2),T,TM,TF
c      WRITE(4,7) NF,NS
      RETURN
